...
BEGIN
    ...
    WHILE i < num_rows DO
        ...
        VALUES (
            ...
            1950 + FLOOR(RAND() * 73),
            CONCAT('Company description for company number ', i + 1),
            ROUND(RAND() * 50000000, 2),
            FLOOR(RAND() * 10000) + 1,
            CONCAT('https://logo.company', i + 1, '.com/logo.png')
        );
        SET i = i + 1;
    END WHILE;
END$$
...
BEGIN
    ...
    WHILE i < num_rows DO
        ...
        VALUES (
            ...
            ROUND(RAND() * 500 + 5, 2),
            ROUND(RAND() * 5 + 0.1, 3),
            CONCAT((10 + FLOOR(RAND() * 90)), 'x', (10 + FLOOR(RAND() * 90)), 'x', (1 + FLOOR(RAND() * 20)), ' cm'),
            ELT(FLOOR(1 + RAND() * 8), 'Black', 'White', 'Silver', 'Gray', 'Blue', 'Red', 'Green', 'Gold', 'Brown'),
            ELT(FLOOR(1 + RAND() * 5), 'Small', 'Medium', 'Large', 'XL', 'Universal'),
            ELT(FLOOR(1 + RAND() * 5), 'Plastic', 'Metal', 'Glass', 'Composite', 'Carbon Fiber'),
            FLOOR(RAND() * 1000),
            FLOOR(RAND() * 50) + 5,
            IF(RAND() > 0.1, TRUE, FALSE),
            CONCAT('https://images.example.com/product', i + 1, '.jpg')
        );
        SET i = i + 1;
    END WHILE;
END$$
...
BEGIN
    ...
    WHILE i < num_rows DO
        ...
        INSERT INTO addresses (CUSTOMER_ID, ADDRESS_TYPE, STREET_ADDRESS, APARTMENT, CITY, STATE_PROVINCE, POSTAL_CODE, COUNTRY, LATITUDE, LONGITUDE, IS_DEFAULT)
        VALUES (
            customer_id_var,
            ELT(FLOOR(1 + RAND() * 5), 'Home', 'Work', 'Billing', 'Shipping', 'Other'),
            CONCAT('Street ', i + 1, ', Building ', FLOOR(RAND() * 100) + 1),
            CASE WHEN RAND() > 0.5 THEN CONCAT('Apt ', FLOOR(RAND() * 300) + 1) ELSE NULL END,
            ELT(FLOOR(1 + RAND() * 10), 'Moscow', 'St. Petersburg', 'Novosibirsk', 'Yekaterinburg', 'Nizhny Novgorod', 'Kazan', 'Chelyabinsk', 'Omsk', 'Samara', 'Rostov-on-Don'),
...
BEGIN
    ...
    WHILE i < num_rows DO
        ...
        INSERT INTO orders (ORDER_NUMBER, CUSTOMER_ID, STATUS, PAYMENT_METHOD, PAYMENT_STATUS, SUBTOTAL, TAX_AMOUNT, SHIPPING_AMOUNT, DISCOUNT_AMOUNT, TOTAL_AMOUNT, CURRENCY, ORDER_DATE)
        VALUES (
...
BEGIN
    ...
    WHILE i < num_rows DO
        ...
        INSERT INTO deals (DEAL_NAME, CUSTOMER_ID, LEAD_ID, STAGE, VALUE, CURRENCY, PROBABILITY, EXPECTED_CLOSE_DATE, ASSIGNED_TO)
        VALUES (
